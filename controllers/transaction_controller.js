    ...
    try {
        ...
        if (oldTransaction) {
            if (oldTransaction.purchase_type === "rented") {
                const transaction = await transactionModel.updateOne({ book_id: oldTransaction.book_id,user_id:oldTransaction.user_id }, {
                    $set: { purchase_type: "bought" }
                }, { new: true });
...
const progress = async(req,res)=>{
    ...
    const getProgress = async(req,res)=>{
        ...
        const currentPageNumber = await progressModel.findOne({user_id:user_id,book_id:book_id})
        console.log(currentPageNumber,"page");
        let val = currentPageNumber!=null ? currentPageNumber.current_page : 0 
